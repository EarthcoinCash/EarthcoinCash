...
const char * const BITCOIN_CONF_FILENAME = "earthcoincash.conf";
const char * const BITCOIN_PID_FILENAME = "earthcoincash.pid";
...
#else
    const char* pszModule = "earthcoincash";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EarthcoinCash";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EarthcoinCash";
#else
    // Unix
    return pathRet / ".earthcoincash";
#endif
#endif
}
